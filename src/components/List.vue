<template>
  <div class="container">
    <div class="px-3 py-3 pt-md-5 pb-md-4 mx-auto">
      <h1 class="mb-2 mr-md-auto font-weight-normal text-center">
        Lists of tasks
      </h1>
      <div class="btn-group float-left">
        <button
          ...
          :class="[
            'btn',
            {
              'btn-primary': selected === state,
              'btn-default': selected !== state
            }
          ]"
        >
          {{ state }}
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="task of tasks" :key="task.id">
              ...
              <td
                :class="
                  task.status === 'active' ? 'text-primary' : 'text-muted'
                "
              >
  ...
  computed: {
    tasks() {
      if (this.selected !== "All tasks") {
        let arr = this.initialTasks.filter(item => {
          if (this.selected === "inactive") {
            return item.status === "inactive" && Form.checkDate(item.date);
          }
          if (this.selected === "expired") {
            return !Form.checkDate(item.date);
          }
          return item.status === this.selected;
        });
        return arr;
      }
